...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  public int indexOf(Position<E> p) {
    ...
    int count =0;
	//initialize a Position object called walk and initialize that to be the output of the first() method
    Position<E> walk= first();
	while (walk != p) {
		//increase count variable by 1
        count++;
		//advance walk to be the next position by calling the after method
        walk=after(walk);
	}
  ...
  public static void main(String[] args){
    ...
    list.addFirst(3);
    list.addFirst(2);
    Position<Integer> pos_1 = list.addFirst(1);

	int index_1 = list.indexOf(pos_1);	
	System.out.println(index_1);

	Position<Integer> pos_2 = list.after(pos_1);
	int index_2 = list.indexOf(pos_2);
	System.out.println(index_2);

	Position<Integer> pos_3 = list.findPosition(1);
	Node<Integer> node = list.validate(pos_3);
